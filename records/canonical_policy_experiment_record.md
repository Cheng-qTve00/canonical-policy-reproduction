...
在完成 Stack D1 后，又选择 NutAssembly D0 作为第二个操作任务进行训练和评估。该实验的目的并不是将其包装为严格的“跨任务泛化实验”，而是进一步验证此前完成的数据处理、Canonical Policy 训练流程和评估流程能否应用到另一个结构和操作要求不同的 MimicGen 任务中。
...
该实验主要用于检验此前完成的数据处理、Canonical Policy 模型、训练流程以及正式评估流程是否能够继续应用到第二个任务，而不是将其定义为严格意义上的跨任务泛化实验。
...
- reward = 1.0：12 个 episode
- reward = 0.5：24 个 episode
- reward = 0：14 个 episode

但同一份日志的末尾又记录了 `test/mean_score: 0.42`。按逐 seed 区块计算，均值应为 `(12×1+24×0.5+14×0)/50=0.48`，与日志均值不一致。你提供的候选分布（18 个 0.5、20 个 0）也无法由原始逐 seed 区块支持。

因此当前可以确认的结果是：完整成功次数为 12 / 50，即完整成功率 24%；日志报告的 mean score 为 0.42。部分 reward 的精确数量存在日志内部冲突，不能据此计算非零 reward 比例，后续需要重新运行评估或修复导出脚本后再确定。

这个结果说明模型并不是在 76% 的场景中完全无法完成任务，而是在相当一部分测试中能够执行到任务的一部分阶段，但没有最终获得完整成功。

所以 NutAssembly 的结果更适合解释为：

当前模型已经学习到了与任务相关的有效操作行为，但在完整任务链条的稳定完成方面仍存在明显不足。
...
NutAssembly 日志显示存在部分完成分数，但由于逐 seed 分布与 mean score 不一致，当前不对 reward=0.5 的比例作定量结论。
...
这一连续过程中。具体失败阶段仍需结合可靠的逐 episode 记录或视频进一步分析。
...
Canonical Policy 在 NutAssembly D0 上的性能就是 24%。

更准确的写法应该是：

在当前训练随机种子和本地实验设置下，CP-SO2 在 NutAssembly D0 的 50 个固定测试 episode 中取得了 24% 的完整成功率和 0.42 的平均得分。
...
随后又完成了 NutAssembly D0 的 CP-SO2 训练和 50 次正式测试。当前 seed43 模型取得 24% 的完整成功率和 0.42 的 mean score，其中日志的部分 reward 分布存在内部冲突，因此不报告非零 reward 比例。
